...

{
    ...
    {
        // User role permission start here
        if(!auth()->user()->canAccess('ChartAccountController.add')){
            return view('permission-denied');
        }
        // User role permission end here
    ...
    {
        // User role permission start here
        if(!auth()->user()->canAccess('ChartAccountController.show')){
            return view('permission-denied');
        }
        // User role permission end here
    ...
    {
        // User role permission start here
        if(!auth()->user()->canAccess('ChartAccountController.edit')){
            return view('permission-denied');
        }
        // User role permission end here
    ...
    {
        // User role permission start here
        if(!auth()->user()->canAccess('ChartAccountController.delete')){
            return view('permission-denied');
        }
        // User role permission end here
    ...
    public function getSales(Request $request) {
        ...
        // $sales = $sales->sales_range(Carbon::parse($daterange[0])->format('Y-m-d'), Carbon::parse($daterange[1])->format('Y-m-d'));
        // $amnt = array();

        // foreach($sales as $sale) {
        //     $amnt[] = $sale->current_balance;
        // }
        // return $amnt;

        $fromDate = Carbon::parse($daterange[0])->format('Y-m-d h:m:s');
        $toDate = Carbon::parse($daterange[1])->format('Y-m-d h:m:s');
        ...
        $sales = DB::table('vouchers')
                ...
                DB::raw('IFNULL(SUM(credit - debit), 0) as current_balance'
                )
                )
        ...
        for($i=$fromDateNum-1;$i<$toDateNum;$i++){
            ...
            if(!empty($taxes)) {
                if($month[$i]==$taxes[$t]->month) {
                    $tax_arr[] = $taxes[$e]->current_balance;
                    $t++;
                }else {
                    $tax_arr[] = 0;
                }
