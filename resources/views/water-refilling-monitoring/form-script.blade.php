<script>
    ...
    $('#prod_ajax').autocomplete({
        source: '{{ url("water-refilling-monitoring/find-product-autosuggest") }}',
        minLength: 2,
    });
    ...
    $('.btn-search, .customer-search_by_id').on('click', function(){
        ...
        $.get('{{url("water-refilling-monitoring/find-customer-only")}}', request, function(datacust){
    ...
    function do_calculation2(){
        ...
        /* Copy info */
        $('[name=return_bottle], [name=order_qty], [name=amount_due]').trigger('change');
    ...
    $('.btn-enter').on('click', function(){
        ...
        var type_entry = $("select.type-entry").val();
        if(type_entry == 0){
            $('.label-select_entry').show(); 
            return;
        }else{
            $('.label-select_entry').hide(); 
        }
        ...
        $.get('{{url("water-refilling-monitoring/find-product")}}', request, function(data){
            if(data && Object.keys(data).length){
                ...
                if(num > 0){
                    //alert('This is to inform you that only one product allow to the entry. Thank you.');
                    $("h3.err-title").html("<strong style='color: red;'>This is to inform you that only 1 product allow to the entry. Thank you.</strong>");
                    return false;
                }
                ...
                // Adding value to hidden input name="type_filter"
                var pro_name = $('[name=search_prod]').val();
                var purified = pro_name.toLowerCase().match("puri");
                var mineral = pro_name.toLowerCase().match("min");
                var alkaline = pro_name.toLowerCase().match("alk");

                if(purified){
                    $('[name=type_filter]').val('purified');
                    //console.log("purified");
                }else if(mineral){
                    $('[name=type_filter]').val('mineral');
                    //console.log('mineral');
                }else if(alkaline){
                    $('[name=type_filter]').val('alkaline');
                    //console.log('alkaline');
                }else{
                    $('[name=type_filter]').val('others');
                }
            }
        }, 'json');
    });
    ...
    $('[name=issued_bottle]').val(0);
    $('[name=return_bottle]').val(0);
    $('[name=order_qty]').val(0);
    $('[name=amount_due]').val('0.00');
    $('[name=refill_bottle]').val(0);
    $('.others_qty').val(0);
    $('.container_qty').val(0);
    $('.dealer_qty').val(0);
    $('.refill_bottle-row').hide();
    $('.container-row').hide();
    $('.dealer-row').hide();
    $('.others-row').hide();
    // Type of entry filter
    $("select.type-entry").change(function(){
        ...
        if(type_entry == 1){
            ...
            $('.searh-div').show();
            $('#tbl-items').show();
            $('.issued_bottle-row').show();
            $('.return_bottle-row').show();
            $('.order_qty-row').show();
            $('.amount_due-row').show();
        }else if(type_entry == 2){
            $('.re_bots').hide();
            $('.issued_bottle-row').hide();
            $('.return_bottle-row').hide();
            $('.container-row').hide();
            $('.dealer-row').hide();
            $('#tbl-items').show();
            $('.order_qty-row').hide();
            $('.amount_due-row').show();
            $('.searh-div').show();
            $('.refill_bottle-row').show();
            $('.order_qty').val('0');
        }else if(type_entry == 3){
            $('.return_bottle-row').show(); 
            $('.issued_bottle-row').show();
            $('#tbl-items').hide();
            $('.order_qty-row').hide();
