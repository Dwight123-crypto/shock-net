<script>
    ...
    $('#service').autocomplete({
        source: '{{ url("billing-invoice/get-services") }}',
        minLength: 2,
    });
    ...
    function rename_fields(){
        var cntr = 0;
        $('#tbl-items tr.serv-row').each(function(){
            $('input', this).each(function(){
                var field_name = $(this).attr('name');
                if( field_name.indexOf("services[") == 0 ){
    ...
    function do_services_total(){
        var gross_sales = 0;
        $('#tbl-items tr.serv-row').each(function(){
            var subtotal = $(this).find('input.price').val() || 0;
        ...
        $('#tbl-items td.total.amount').text( gross_sales.toFixed(2) );
    }
    ...
    $('.btn-get_service').on('click', function(){
        var s = ($('#service').val()).trim();

        if( !s ) return;

        var request = { s: s };
        
        var search_ui = $('#service').data('ui-autocomplete');
        if( search_ui && search_ui.selectedItem ){
            request.serv_id = search_ui.selectedItem.id;
        }
        
        $.get('{{url("billing-invoice/get-service")}}', request, function(data){
            
            if(data && Object.keys(data).length){
                /* Add tr */
                var html = $('.serv-row-template tbody').html();
                var num  = $('.table-service-info tr.serv-row').length;
                html = html.replace(new RegExp('{num}', 'g'), num);
                $('.table-service-info > tbody').append( html );
                
                /* Fill in values to new tr */
                var $tr = $('.table-service-info tr.serv-row:last-child');
                $tr.find('input.service_id').val( data.id );
                $tr.find('input.name').val( data.name );
                // $tr.find('input.subtotal').val( parseFloat(data.rate).toFixed(2) );
                $tr.find('input.price').val( parseFloat(data.rate).toFixed(2) );
                console.log(parseFloat(data.rate).toFixed(2));
                /* Calculation */
                do_services_total();
            }
    ...
    $('.table-service-info').on('change', 'input.price', function(){
        var price = $(this).val() || 0;
        price = isNaN(price) ? 0 : parseFloat(price).toFixed(2)*1;
        
        var $tr = $(this).closest('tr');
        // var qty = $tr.find('input.qty').val() || 0;
        // $tr.find('input.price').val( parseFloat(price * qty).toFixed(2) );
        $tr.find('input.price').val( parseFloat(price).toFixed(2) );
